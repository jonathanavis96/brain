...
try:
    # When executed as a package module: python3 -m projects.ai_integration_audit.generator.auditgen
    ...
    # When executed as a script: python3 projects/ai_integration_audit/generator/auditgen.py
    from followup_scoring import parse_followup_answers, score_opportunity
    from reporting import RankedOpportunity, followup_intake_form_schema, render_executive_report
    from research_providers import ResearchDossier, get_provider
    from scoring import OpportunityScores, derived_low_hanging, derived_strategic
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.intake:
        ...
        if args.base_intake:
            intake = load_intake(Path(args.base_intake))
        else:
            # Minimal baseline if no base intake is provided.
            intake = Intake(
