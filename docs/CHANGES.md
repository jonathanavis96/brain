...
### Files Changed
- `workers/ralph/loop.sh` - Added stale lock detection and model defaults
...
1. Replace `ralph/PROMPT.md` with lean version from `templates/ralph/PROMPT.project.md`
2. Replace `ralph/loop.sh` with thin wrapper from `templates/ralph/loop.sh`
