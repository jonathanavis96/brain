# Premium UI/UX Audit Prompt (Jobs/Ive) — Design-Only

This document contains the canonical **design-only audit prompt** used to run a premium UI/UX review.

- **Scope:** Visual design + interaction design only. No feature changes.
- **Output:** A phased plan for approval (Phase 1–3) + deterministic implementation notes.

> Note: This is intentionally a repo-level workflow artifact (not a “skill”).
...
(You do not need Ralph outputs to run a design audit; Cortex should produce the plan and update `workers/IMPLEMENTATION_PLAN.md` for Ralph.)
