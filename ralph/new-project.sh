...
if [[ "$create_repo_answer" =~ ^[Yy] ]]; then
    ...
    if ! check_gh; then
        ...
        echo ""
        read -p "Continue local-only? (y/n): " local_only_answer
    ...
    if [[ "$CREATE_REPO" == "true" ]]; then
        ...
        if [[ -n "$detected_username" ]]; then
            echo ""
            read -p "GitHub username [$detected_username]: " input_username
        ...
        echo -e "${CYAN}----------------------------------------${NC}"
        echo ""
        read -p "Proceed with this configuration? (y/n): " confirm_answer
    ...
    while true; do
        ...
        if [[ $create_exit -eq 0 ]]; then
            ...
            if echo "$create_output" | grep -qiE "already exists|name.*(taken|unavailable)|repository creation failed|could not create repository"; then
                ...
                echo ""
                read -p "Enter a different repository name (or 'skip' for local-only): " new_repo_name
