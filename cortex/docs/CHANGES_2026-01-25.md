...
| `workers/ralph/sync_cortex_plan.sh` | Syncs Cortex→Ralph + runs cleanup | ✅ Implemented |
...
- `sync_cortex_plan.sh` automatically calls `cleanup_plan.sh --archive` after syncing
- Use `sync_cortex_plan.sh --no-cleanup` to skip cleanup (debugging)
- Runs automatically when Ralph's `loop.sh` executes (via sync_cortex_plan.sh)
...
### Additional Fix (2026-01-25 18:10)

**Issue:** `sync_cortex_plan.sh` only ran cleanup after successful sync with new sections. Early exits skipped cleanup.

**Fix:** Added cleanup calls to all early exit paths:

- When Ralph's plan is already current (no updates needed)
- When no new sections found in Cortex plan

**Result:** PLAN mode now ALWAYS cleans up completed tasks, regardless of sync status.
