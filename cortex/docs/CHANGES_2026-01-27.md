...
> **⚠️ HISTORICAL DOCUMENT** - This is a historical change log from 2026-01-27. For current work status, see [workers/IMPLEMENTATION_PLAN.md](../../workers/IMPLEMENTATION_PLAN.md) and [workers/ralph/THUNK.md](../../workers/ralph/THUNK.md).
...
## (external knowledge-app app): canonical spec + implementation planning
...
- **Canonical spec (authoritative design reference):**
  - `docs/knowledge-app/knowledge-app-spec.md`
...
- **Repo layout + integration section:**
  - Canonical folder structure under `external/knowledge-app/`
...
- **Implementation plan:**
  - `docs/knowledge-app/knowledge-app-implementation-plan.md`
  - MVP cutline + phased (MVP/V1/V2) atomic Ralph task breakdown (no code).

### Commits ((external knowledge-app app))

- `0ed0b7b` — docs(knowledge-app): add canonical brain map specification
- `1400e83` — docs(knowledge-app): specify canonical frontmatter and link schema
- `917b2e4` — docs(knowledge-app): define canonical repo layout and integration
- `f52bd09` — docs(knowledge-app): add REST API graph query contracts
- `adff55d` — docs(knowledge-app): add implementation plan
...
- Linting:
  - `markdownlint docs/knowledge-app/knowledge-app-spec.md`
  - `markdownlint docs/knowledge-app/knowledge-app-implementation-plan.md`
- Git operations:
  - `git add <file>`
  - `git commit -m "..."`
  - `git log --oneline -n <N>`
...
- (external knowledge-app app) work was committed as docs-only changes, and kept logically isolated from unrelated working tree changes.
