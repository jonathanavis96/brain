...
EOF
  ...
  if ! echo "$summary_block" | grep -qE '\*\*Summary\*\*|\*\*Changes Made\*\*|\*\*Next Steps\*\*'; then
    ...
    # Provide Discord-friendly metadata + excerpt
    local excerpt
    excerpt=$(tail -40 "$logfile" | sed $'s/\x1b\[[0-9;]*m//g' | sed '/^[─═]+$/d; /^STATUS |/d; /^PROGRESS |/d')
...
**Recent Log Excerpt** (last ~40 lines):
...
# Test with the fallback fixture
generate_iteration_summary "5" "build" "tests/fixtures/summary_case5_fallback_needed.log"
