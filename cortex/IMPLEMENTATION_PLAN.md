...
- [x] **26.2** Verify all Brain Map backend tests pass
  - **Goal:** Confirm Phase 25 implementation quality
  - **Dependencies:** 26.1
  - **Steps:**
    1. Activate venv: `source app/brain-map/backend/.venv/bin/activate`
    2. Run full test suite: `pytest app/brain-map/backend/tests/ -v`
...
- [x] **26.3** Add Brain Map test running to verifier (optional check)
  - **Goal:** Include Python tests in quality gates (as WARN initially)
  - **Dependencies:** 26.2
  - **Scope:**
    - Add pytest check to verifier.sh (gate=warn until stable)
    - Only run if venv exists (skip if not setup)
  - **Acceptance:**
    - Verifier runs pytest if venv found
    - Shows [WARN] if tests fail, [PASS] if succeed, [SKIP] if no venv
