...
- [x] **0.L.2** Fix MD056 table column count errors in TEMPLATE_DRIFT_REPORT.md
...
- [x] **0.L.3** Fix MD032 blank line errors in TEMPLATE_DRIFT_REPORT.md
...
- [x] **0.L.4** Fix MD009 trailing spaces in TEMPLATE_DRIFT_REPORT.md
...
- [x] **0.L.5** Fix MD040 missing language in TEMPLATE_DRIFT_REPORT.md
  - **AC:** `markdownlint TEMPLATE_DRIFT_REPORT.md` passes (no MD040 errors)
  - **Location:** Line 247
  - **Fix:** Add language tag to fenced code block (```bash,```text, etc.)

- [x] **0.L.6** Fix MD056 table column count errors in workers/ralph/THUNK.md
...
- [x] **25.1.1** Scaffold Brain Map app workspace (frontend + backend skeleton)
...
- [x] **25.1.2** Define Markdown note discovery + loading (scan notes root)
...
- [x] **25.1.3** Implement canonical frontmatter parser and validator
...
- [x] **25.1.4** Build SQLite schema and deterministic index rebuild
...
- [x] **25.1.5** Implement `/search` endpoint using the index (fast global search)
...
- [x] **25.1.6** Implement `/graph` endpoint (graph snapshot for UI)
...
- [x] **25.1.7** Implement `/node/{id}` read endpoint
...
- [x] **25.1.8** Implement node create (`POST /node`) with markdown-first write + reindex
...
- [x] **25.1.9** Implement node update (`PUT /node/{id}`) with atomic write + reindex
...
- [x] **25.1.10** Implement `/generate-plan` endpoint (deterministic markdown + optional write)
...
- [x] **25.1.11** Frontend: Graph view rendering from `/graph` (sigma.js)
...
- [x] **25.1.12** Frontend: Node detail panel (read + edit + save)
...
- [x] **25.1.13** Frontend: Search palette (Ctrl+K) via `/search`
