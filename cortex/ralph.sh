#!/usr/bin/env bash
# ralph.sh - User-friendly wrapper for Brain repo Ralph

set -euo pipefail

# Fixed path to Brain repo (adjust if needed)
BRAIN_ROOT="$HOME/code/brain"
LOOP_SH="$BRAIN_ROOT/workers/ralph/loop.sh"
...
Short Options:
  ...
  -m MODEL             Model to use (e.g., gpt52, gpt-5.2-codex, auto)
...
Examples:
  ...
  ralph -m gpt52 -i10        # Use GPT-5.2 Codex, 10 iterations
...
if [[ ! -x "$LOOP_SH" ]]; then
  echo "Error: loop.sh not found or not executable at $LOOP_SH" >&2
  exit 1
fi
