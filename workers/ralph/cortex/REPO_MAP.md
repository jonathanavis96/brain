...
**Purpose:** Curated reference materials (e.g., React best practices, performance optimization).

**Current References:**
- `react-best-practices/` - React/Next.js performance guidelines
  - `INDEX.md` - Categorized rule index
  - `HOTLIST.md` - Most commonly applicable rules
  - `rules/` - 45+ individual rule files

**Usage:** Consult HOTLIST first, then drill down to specific rules as needed.
...
- Generator scripts in `../generators/` folder
...
**Purpose:** Scripts to generate common project files from templates.

**Scripts:**
- `generate-neurons.sh` - Generate NEURONS.md from template
- `generate-thoughts.sh` - Generate THOUGHTS.md from template
- `generate-implementation-plan.sh` - Generate IMPLEMENTATION_PLAN.md from template
...
**Purpose:** Third-party reviews and analysis (e.g., CodeRabbit).
...
| `AGENTS.md` | Operational guide for agents | Human/Cortex |
| `NEURONS.md` | Root-level codebase map | Ralph/Workers |
| `THOUGHTS.md` | Strategic goals and decisions | Cortex |
| `IMPLEMENTATION_PLAN.md` | Active task list | Cortex → Ralph |
...
| `rovodev-config.yml` | RovoDev configuration | Human |

## Where State Lives

- **Current tasks:** `IMPLEMENTATION_PLAN.md` (root and `cortex/`)
- **Completed tasks:** `THUNK.md`
- **Strategic context:** `THOUGHTS.md` (root and `cortex/`)
...
- **Maintenance items:** `.maintenance/MAINTENANCE.md`
...
- **skills/ total:** 30+ files (see NEURONS.md for exact count)
- **templates/ total:** ~30 files across subdirectories
- **references/react-best-practices/rules/:** 45 files
- **cortex/:** Growing (Phase 0-A in progress)
- **workers/:** 1 worker (ralph/) currently

## Last Updated

2026-01-21 - Created as part of Phase 0-A.1.3 (Cortex Manager Pack)
