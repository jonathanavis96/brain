...
# =============================================================================
# Context Management (prevent unbounded token growth)
# =============================================================================
...
# =============================================================================
# Task Extraction (focus agent on ONE task)
# =============================================================================


def extract_next_task(cwd: str | None = None) -> str | None:
    """
    Extract the next unchecked TOP-LEVEL task from IMPLEMENTATION_PLAN.md.
    Top-level tasks start with "- [ ] **X.Y**" (have a task ID).
    Returns the task line and any sub-bullets, or None if no tasks found.
    """
    work_dir = Path(cwd or ".")

    # Try both locations
    plan_paths = [
        work_dir / "workers" / "ralph" / "IMPLEMENTATION_PLAN.md",
        work_dir / "IMPLEMENTATION_PLAN.md",
    ]

    import re

    task_pattern = re.compile(r"^- \[ \] \*\*\d+\.\d+\*\*")  # e.g., "- [ ] **9.2**"
    ...
    for plan_path in plan_paths:
        ...
        for line in lines:
            # Found a top-level unchecked task (has task ID like **9.2**)
            if task_pattern.match(line.strip()) and not in_task:
                task_lines = [line]
                in_task = True
            # Continuation lines (indented sub-bullets)
            elif in_task and line.startswith("  ") and line.strip():
                task_lines.append(line)
            # Next top-level item or blank line ends current task
            elif in_task and (line.startswith("- ") or line.strip() == ""):
                break
...
class Agent:
    ...
    def run(self, prompt: str, max_turns: int = DEFAULT_MAX_TURNS) -> bool:
        ...
        # Extract the ONE task to focus on
        next_task = extract_next_task(self.cwd)
        if next_task:
            task_focus = f"```\n{next_task}\n```"
            print(
                f"  {Style.DIM}Task:{Style.RESET}     {next_task.split(chr(10))[0][:60]}..."
            )
        else:
