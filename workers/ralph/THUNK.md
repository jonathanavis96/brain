...
| 504 | 0.W.3 | HIGH | **0.W.3** Sync current_ralph_tasks.sh to templates/ralph/ (Hygiene.TemplateSync.1) - Copied workers/ralph/current_ralph_tasks.sh to templates/ralph/, files now identical (diff returns no output), workers version includes performance optimizations (pure bash string manipulation vs sed/cut subprocesses) | 2026-01-23 |
...
| 515 | 3.5.1 | HIGH | **3.5.1** Audit workers/ralph/ scripts for hardcoded paths - Fixed hardcoded "brain/ralph" paths in loop.sh (lines 1109, 1202) to "workers/ralph", updated .verify/ paths to use ../.verify/ prefix for correct relative pathing from workers/ralph/ directory, all scripts now use $ROOT or relative paths consistently | 2026-01-23 |
...
| 514 | 5.1.4 | MEDIUM | **5.1.4** Update `templates/cortex/CORTEX_SYSTEM_PROMPT.project.md` - Verified all path references already use "workers/ralph/" format (no bare "ralph/" paths found with negative lookbehind grep), AC satisfied | 2026-01-24 |
