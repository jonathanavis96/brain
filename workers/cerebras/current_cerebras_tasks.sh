#!/bin/bash
# Cerebras Task Monitor - Real-time display of IMPLEMENTATION_PLAN.md tasks
#
# Usage: bash current_cerebras_tasks.sh [--hide-completed]
#
# Features:
...
CEREBRAS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLAN_FILE="$CEREBRAS_DIR/../IMPLEMENTATION_PLAN.md"
THUNK_FILE="$CEREBRAS_DIR/THUNK.md"
...
if [[ ! -f "$PLAN_FILE" ]]; then
  echo "Error: IMPLEMENTATION_PLAN.md not found in $CEREBRAS_DIR"
  exit 1
fi
...
show_help() {
  clear
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║              CURRENT CEREBRAS TASKS - HELP                        ║"
  echo "╚════════════════════════════════════════════════════════════════╝"
  echo ""
  echo "  HOTKEYS:"
  echo ""
...
display_tasks() {
  ...
  # Header
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║          CURRENT CEREBRAS TASKS - $(date +%H:%M:%S)                  ║"
  echo "╚════════════════════════════════════════════════════════════════╝"
  echo ""
...
echo "Starting Cerebras Task Monitor..."
...
cleanup() {
  ...
  echo ""
  echo "Exiting Cerebras Task Monitor..."
  exit 0
}
