#!/bin/bash
# Thunk Cerebras Tasks Monitor - Persistent completion log display
#
# Usage: bash thunk_cerebras_tasks.sh
#
# Features:
#   - Displays completed tasks from THUNK.md (Cerebras appends when marking tasks complete)
#   - Groups tasks by Era
...
if [[ ! -f "$THUNK_FILE" ]]; then
  echo "Error: THUNK.md not found in $CEREBRAS_DIR"
...
display_thunks() {
  ...
  # Header
  echo "╔════════════════════════════════════════════════════════════════╗"
  echo "║          THUNK CEREBRAS TASKS - $(date +%H:%M:%S)                     ║"
  echo "╚════════════════════════════════════════════════════════════════╝"
  echo ""
...
# Main loop
echo "Starting Thunk Cerebras Tasks Monitor..."
...
cleanup() {
  ...
  echo ""
  echo "Exiting Thunk Cerebras Tasks Monitor..."
  exit 0
}
