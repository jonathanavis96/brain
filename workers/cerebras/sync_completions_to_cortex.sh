#!/usr/bin/env bash
# sync_completions_to_cortex.sh - Sync task completions from Cerebras to Cortex
#
# Purpose: Propagate task completion status ([x]) from Cerebras's execution plan
...
#
# Protocol: Complements sync_cortex_plan.sh (which syncs Cortex → Cerebras)

set -euo pipefail

# Paths (relative to workers/cerebras/)
CORTEX_PLAN="../../cortex/IMPLEMENTATION_PLAN.md"
CEREBRAS_PLAN="../IMPLEMENTATION_PLAN.md"
...
if [[ ! -f "$CEREBRAS_PLAN" ]]; then
  log_verbose "Cerebras plan not found: $CEREBRAS_PLAN - skipping reverse sync"
  exit 0
fi

# Extract completed task IDs from Cerebras's plan
...
while IFS= read -r line; do
  if [[ "$line" =~ ^[[:space:]]*-[[:space:]]\[x\][[:space:]]\*\*([0-9]+\.[0-9A-Z.]+)\*\* ]]; then
    ...
    log_verbose "Cerebras completed: $task_id"
  fi
done <"$CEREBRAS_PLAN"

if [[ $completed_count -eq 0 ]]; then
  log_info "0 completions found in Cerebras's plan"
  exit 0
fi

log_info "Found ${#completed_tasks[@]} completed tasks in Cerebras's plan"
...
while IFS= read -r line; do
  # Check if this is an unchecked task that Cerebras has completed
