...
#   workers/cerebras/THUNK.md using a strict, schema-safe markdown table format.
#
# Why:
#   Prevent MD056 table column mismatch errors caused by ad-hoc THUNK edits.
#   This script is designed to be safe to run automatically between loop iterations.
#
# Behavior:
...
#   - workers/cerebras/THUNK.md
#
# Output:
#   - Appends 5-column rows to THUNK.md: | THUNK # | Original # | Priority | Description | Completed |
#
# Usage:
#   bash workers/cerebras/update_thunk_from_plan.sh
#   bash workers/cerebras/update_thunk_from_plan.sh --dry-run
...
THUNK_FILE="${REPO_ROOT}/workers/cerebras/THUNK.md"
