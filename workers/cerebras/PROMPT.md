# Cerebras Loop - Brain Repository Self-Improvement
...
**Start:** `STATUS | branch=<branch> | runner=cerebras | model=<model>`
...
2. **After reading workers/IMPLEMENTATION_PLAN.md, your VERY NEXT step must be:**
   ...
   - NOT re-reading workers/IMPLEMENTATION_PLAN.md again
...
1. **CHECK FOR VERIFIER WARNINGS FIRST:**
   - If `workers/workers/IMPLEMENTATION_PLAN.md` has a "## Verifier Warnings" section with unchecked `- [ ]` tasks:
     ...
     - Commit with message: `fix(cerebras): resolve verifier warning <RULE_ID>`
...
5. **SINGLE COMMIT RULE:** Commit ALL changes together (code fix + workers/ralph/workers/ralph/THUNK.md + workers/IMPLEMENTATION_PLAN.md):
   ...
   Co-authored-by: cerebras-brain <cerebras-brain@users.noreply.github.com>
...
**You have access to the ENTIRE brain repository** (from `$ROOT`), not just `workers/cerebras/`.
...
When fixing issues, search the ENTIRE repo: `rg "pattern" $ROOT` not just `workers/cerebras/`.
...
- **Protected files** - Do NOT modify: `rules/AC.rules`, `../.verify/ac.sha256`, `verifier.sh`, `../.verify/verifier.sha256`, `loop.sh`, `../.verify/loop.sh.sha256`, `PROMPT.md`, `../.verify/prompt.sha256`
...
- `tail workers/cerebras/workers/ralph/THUNK.md` - get next number ONCE
...
`<type>(<scope>): <summary>` where type is `feat|fix|docs|refactor|chore|test` and scope is `cerebras|templates|skills|plan`.
